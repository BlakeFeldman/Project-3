...
module.exports = function(app) {
  app.get("/api/go-fund-me", function(req, res) {
    // gfm.setCampaignUrl("https://www.gofundme.com/gemma-paybarah");
    ...
    axios
      .get(
        "https://www.gofundme.com/mvc.php?route=homepage_norma/search&term=special%20needs%20children"
      )
      .then(function(response) {
        ...
        $(".grid-item").each(function(i, element) {
          ...
          result.title = $(element)
            .find(".fund-title.truncate-single-line")
            .text();
          ...
          var goal = $(element)
            .find(".fund-item")
            .text();
          var reg = /[\$£]([0-9,]+?) raised of [\$£]([0-9,]+)/;
          var matches = goal.match(reg);
          var donationGoal = {};
          if (matches) {
            donationGoal = {
              raised: matches[1],
          ...
          result.lastDonation = "last donation test";
          result.donationGoal = donationGoal;
          result.link = $(element)
            .find("a")
            .attr("href");
